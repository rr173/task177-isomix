...
func EvaluateStandard(bounds map[string]model.Range, now func() time.Time) Summary {
	...
	return Summary{Baseline: baseline(bounds), Conservative: conservative, Expansive: expansive, MostSensitive: MostSensitive(NormalizeRows(all)), GeneratedAt: now().UTC()}
}
